...
а втор елемент е класата како стринг. Да се истренира прв класификатор со користење на стандардната getwords функција
врз основа на тренинг множеството. Исто така, да се искористи дадената функција getwords2 за креирање на втор класификатор.
Вториот класификатор треба да го употреби истото тренинг множество, но притоа ќе ја користи и предефинираната листа на
зборови кои треба да се занемарат при тренирањето. Доколку некој од овие зборови се наоѓа во документот истиот треба да се игнорира,
што е веќе имплементирано во функцијата get_words_ignore. Вие треба само да ја употребите.

Потоа за секој документ прочитан од стандарден влез да се испечатат 2 реда. Првиот ред ја содржи предвидената класа со
првиот класификатор и логаритам со основа 2 од веројатноста со која се предвидува (заокружено на 4 децимали),
а вториот ред предвидената класа со помош на вториот класификатор и логаритам со основа 2 од веројатноста со која се
предвидува (заокружено на 4 децимали). Да се испечати колку пати втората веројатност е поголема од првата заокружено на
4 децимали. Ако предвидувањето на двата класификатори е различно да се испчати уште еден ред со зборот “kontradikcija”.
...
def getwords2(doc):
    splitter = re.compile('\\W')
    words_to_ignore = ['and', 'are', 'for', 'was', 'what', 'when', 'who', 'but', 'from', 'after', 'out', 'our', 'my',
                       'the',
...
if __name__=="__main__":
    ...
    c2 = naivebayes(getwords2)
    for d in train_data:
        c2.train(d[0], d[1])

    doc = input()
    klasa1, v1 = c1.classifyDocument(doc)
    klasa2, v2 = c2.classifyDocument(doc)

    print(klasa1, round(math.log(v1,2),4))
    print(klasa2, round(math.log(v2,2),4))
