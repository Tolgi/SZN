...
if __name__=="__main__":
    ...
    c = naivebayes(getwords)
    trainData = train_data[:200]
    for d in trainData:
        c.train(d[1], d[0])

    testCase = ('Good morning Kimmy :) @KimberlyKWyatt','positive',)

    klasa = c.classifyDocument(testCase[0])
    if klasa == testCase[1]:
        print('Tocno:', testCase[1],',', 'Predvideno: ', klasa,',', 'Tocna klasifikacija: True')
    else:
        print('Tocno:', testCase[1],',', 'Predvideno: ', klasa,',', 'Tocna klasifikacija: False')

    trainData2 = train_data[-5:]

    for d in trainData2:
        klasa = c.classifyDocument(d[1])
        if klasa == d[0]:
            print('Tocno:', d[0], ',', 'Predvideno: ', klasa, ',', 'Tocna klasifikacija: True')
        else:
            print('Tocno:', d[0],',', 'Predvideno: ', klasa,',', 'Tocna klasifikacija: False')
