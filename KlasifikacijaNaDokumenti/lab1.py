...
class naivebayes(documentClassifier):
    ...
    def getCategoryProbabilityForDocument(self, item, currentCategory):
        ...
        return caclulateDocumentProbabilityInClass * catprob
...
if __name__ == "__main__":
    ...
    d = naivebayes(getwords)
    for item in train_data:
        lista = list(item)
        d.train(lista[0], lista[1])

    recenica = """Particularly for the strikers whose hype and profiles eclipse their prowess in front of goal. """

    klasa = d.classifyDocument(recenica)
    print("{0}  {1:.8f}".format(klasa[0], klasa[1]))
